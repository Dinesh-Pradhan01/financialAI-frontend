...
const REG: Record<string, IconDef> = {
  ...
  // business OPEX categories
  payroll: { icon: Banknote, color: "var(--brand-primary)" },
  salary: { icon: Banknote, color: "var(--brand-primary)" },
  rent: { icon: Building2, color: "var(--severity-low)" },
  "office-rent": { icon: Building2, color: "var(--severity-low)" },
  software: { icon: Laptop, color: "var(--brand-secondary)" },
  "software-subscription": { icon: Laptop, color: "var(--brand-secondary)" },
  subscription: { icon: Repeat, color: "var(--brand-secondary)" },
  "cloud-services": { icon: Cloud, color: "var(--brand-primary-hi)" },
  "office-supplies": { icon: Paperclip, color: "var(--severity-moderate)" },
  equipment: { icon: MonitorSmartphone, color: "var(--severity-low)" },
  electronics: { icon: MonitorSmartphone, color: "var(--severity-low)" },
  utilities: { icon: Zap, color: "var(--severity-moderate)" },
  internet: { icon: Wifi, color: "var(--success)" },
  telecom: { icon: Phone, color: "var(--brand-secondary)" },
  "home-services": { icon: Wrench, color: "var(--text-secondary)" },
  shipping: { icon: Truck, color: "var(--severity-low)" },
  delivery: { icon: Truck, color: "var(--severity-low)" },
  "professional-services": { icon: Briefcase, color: "var(--brand-primary)" },
  gst: { icon: Receipt, color: "var(--severity-moderate)" },
