...
import type { InviteRecord, InviteSendPayload, InviteSendResponse } from "@/shared/types/api";
...
export function InviteModal({ isOpen, onClose, invitesEndpoint }: InviteModalProps) {
  ...
  const { data: invites = [], isLoading: loadingInvites } = useQuery({
    queryKey,
    queryFn: () => api.get<InviteRecord[]>(endpoint),
    enabled: isOpen,
    staleTime: 60 * 1000,
  });
  ...
  const sendInviteMutation = useMutation({
    ...
    onSuccess: (res, vars) => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.business.invites() });
    },
    onError: (err: unknown) => {
      console.error(err);
