...
import { parseApiError, getApiErrorMessage } from "@/shared/lib/apiError";
import type { InviteVerifyResponse } from "@/shared/types/api";
...
function AcceptInvitePage() {
  ...
  // Flow B: New account / password setup
  ...
  return (
    <div className="relative grid min-h-screen md:grid-cols-2 bg-white overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col justify-center px-6 py-12 sm:px-12 md:px-16 lg:px-20 bg-white">
        ...
        >
          {/* Mobile Brand Header */}
          <SpotLiteBrand size="sm" className="md:hidden mb-6" />
          ...
          {/* Logged in User Fast Acceptance Option */}
          {user && (
            <div className="mt-5 rounded-2xl border border-brand/30 bg-brand/5 p-4 space-y-2.5">
              ...
              >
                ...
                <span>Accept Invite as {user.full_name || user.email}</span>
              </button>
            </div>
          )}
      ...
      <AuthHeroPanel role={inviteData?.role} companyName={inviteData?.company_name || undefined} />
    </div>
  );
}
