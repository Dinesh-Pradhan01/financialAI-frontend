...
const SECTORS = [
  { id: "fast_moving_consumer_goods", name: "Fast Moving Consumer Goods (FMCG)" },
...
export function IndustryDashboard() {
  ...
  const sectorName = search?.sector_name || "fast_moving_consumer_goods";

  const { data: sectorData } = useQuery(sectorQueryOptions(sectorName));
  const { data: stockData } = useQuery(top5StocksQueryOptions());

  const currentSector = SECTORS.find((s) => s.id === sectorName) || SECTORS[0];
  ...
  const treemapData = useMemo(() => {
    return sortedCompanies.slice(0, 10).map(c => ({
      name: c.name,
      size: c.market_cap_cr || 0,
      market_cap_cr: c.market_cap_cr || 0,
    }));
  }, [sortedCompanies]);
    ...
    <div className="px-5 py-6 md:px-10 space-y-8 pb-16">
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Select value={currentSector.id} onValueChange={handleSectorChange}>
        ...
        <CardHeader className="px-6 py-5 flex flex-row items-center justify-between gap-4 border-b border-border/40">
          <div>
            ...
            <p className="text-xs text-text-secondary mt-0.5">
              Comprehensive list of peer companies in the {currentSector.name} sector.
            </p>
          </div>
