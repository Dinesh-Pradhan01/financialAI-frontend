...
export function IndustryDashboard() {
  ...
  const savedSector =
    typeof window !== "undefined" ? localStorage.getItem("spotlite_selected_sector") : null;
  ...
  const { data: basicIndustries = [], isLoading: isBasicLoading } = useQuery(
    basicIndustriesQueryOptions(),
  );
  const {
    data: sectorData,
  ...
  return (
    <div className="px-5 py-6 md:px-10 space-y-8 pb-16">
      ...
      {isInitialLoading ? (
        ...
        <div
          className={cn("space-y-8 transition-opacity duration-200", isUpdating && "opacity-60")}
        >
