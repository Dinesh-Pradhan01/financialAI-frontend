...
export const generalInfoSchema = z.object({
  ...
  business_type: z.string().min(1, "Please select a business legal type"),
  ...
  business_pan: z
    ...
    .regex(
      /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/,
      "Business PAN must be exactly 10 characters (e.g. ABCDE1234F)",
    ),
  udyam_number: z.string().trim().nullable().optional(),
  date_of_incorporation: z.string().trim().nullable().optional(),
  registered_address: z.string().trim().min(5, "Registered address must be at least 5 characters"),
  ...
  official_email: z.string().trim().email("Please enter a valid official email address"),
  official_phone: z.string().trim().min(10, "Official phone must be at least 10 digits"),
});
...
export const leadershipInfoSchema = z.object({
  ...
  cfo_email: z.string().trim().email("Invalid CFO email").or(z.literal("")).nullable().optional(),
  ...
  hr_email: z.string().trim().email("Invalid HR email").or(z.literal("")).nullable().optional(),
...
export const financialInfoSchema = z.object({
  ...
  number_of_accounts: z.number().int().min(1, "Number of accounts must be at least 1").default(1),
