...
import { resetEmployee, setEmployeePreview } from "@/shared/store/slices/hrSlice";
import { useEmployeeImport } from "../../hooks/useEmployee";
...
import type { EmployeePreviewResponse } from "../../types/employee";

export function EmployeeStickyFooter({
  recordCount,
  errorCount,
  backendPreview,
}: {
  recordCount: number;
  errorCount: number;
  backendPreview: EmployeePreviewResponse;
}) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await employeeApi.previewManual(backendPreview.records);
      ...
      >
        <DialogContent className="max-w-sm text-center p-6">
          ...
          <p className="my-4 text-text-secondary text-sm">
            Successfully imported {recordCount} employee records into the system.
          </p>
          <button
            onClick={handleCancel}
